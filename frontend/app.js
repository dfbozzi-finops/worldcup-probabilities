...
// Endpoints mapping to the JSON structures generated by the Quantitative Pipeline
const ENDPOINTS = {
...
async function fetchPipelineData() {
    try {
        ...
        const [oppsRes, matchRes, propsRes] = await Promise.all([
            fetch(ENDPOINTS.opportunities).catch(err => { throw new Error("Opportunities: " + err.message); }),
            fetch(ENDPOINTS.matches).catch(err => { throw new Error("Matches: " + err.message); }),
            fetch(ENDPOINTS.props).catch(err => { throw new Error("Props: " + err.message); })
        ]);

        const opps = (oppsRes && oppsRes.ok) ? await oppsRes.json() : null;
        const matches = (matchRes && matchRes.ok) ? await matchRes.json() : null;
        const props = (propsRes && propsRes.ok) ? await propsRes.json() : null;
